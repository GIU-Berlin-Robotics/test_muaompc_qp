...
void mpc_mtx_multiply_mtx_vec(real_t pout[], const real_t pmtx[],
        const real_t pvec[],
        const uint32_t rows,
        const uint32_t cols)
{
 ...
 * pmtx has size (rows x columns) */
void mpc_mtx_scale(real_t pout[], const real_t pmtx[],
...
void mpc_mtx_scale_direct(real_t pout[],
        const real_t factor, const uint32_t rows,
        const uint32_t cols)
{
    uint32_t k; /* matrix index (row * column) */
...
void mpc_mtx_add(real_t pmtxc[], const real_t pmtxa[],
...
void mpc_mtx_add_direct(real_t pmtxa[], const real_t pmtxb[],
 ...
 * pmta, pmtb, pmtc have size (rows x columns) */
void mpc_mtx_substract(real_t pmtxc[], const real_t pmtxa[],
        const real_t pmtxb[], const uint32_t rows,
        const uint32_t cols)
{
    uint32_t k; /* matrix index (row * column) */
 ...
 * pmta, pmtb have size (rows x columns) */
void mpc_mtx_substract_direct(real_t pmtxa[],
        const real_t pmtxb[], const uint32_t rows,
        const uint32_t cols)
{
    uint32_t k; /* matrix index (row * column) */
...
void mpc_mtx_mul_add(real_t pvecc[], real_t pvecab[], const real_t pmtxa[],
        const real_t pvecb[], const uint32_t rows,
        const uint32_t cols)
{
    uint32_t k; /* vector index (rows) */
  mpc_mtx_multiply_mtx_vec(pvecab, pmtxa, pvecb, rows, cols);
    for (k = 0; k < rows; k++) {
        pvecc[k] += pvecab[k];
    }
    return;
...
void mpc_mtx_saturate_vec(real_t pvec[], const real_t plower[],
        const real_t pupper[],
        const uint32_t rows)
{
...
void mpc_mtx_max_vec_zero(real_t pmax[], const uint32_t rows)
{
    uint32_t i; /* vector index (row number) */
    real_t zero = 0.;
    for (i = 0; i < rows; i++) {
...
void mpc_mtx_min_vec_zero(real_t pmin[], const uint32_t rows)
{
    uint32_t i; /* vector index (row number) */
    real_t zero = 0.;
    for (i = 0; i < rows; i++) {
...
void mpc_mtx_transpose(real_t * mtxout, const real_t * mtxin,
        const uint32_t rows, const uint32_t cols)
{
    uint32_t i; /* row number */
    uint32_t j; /* column number */
...
void mpc_mtx_shift_sequence(real_t outseq[], const real_t seq[],
...
void mpc_mtx_to_eye(real_t mtx[], const uint32_t dim)
{
    uint32_t i, j;
    for (i = 0; i < dim; i++){
        for (j = 0; j < dim; j++)
...
void mpc_mtx_to_zero(real_t mtx[], const uint32_t l)
{
    uint32_t i;
    for (i = 0; i < l; i++)
        mtx[i] = 0.;
