...
extern void mpc_mtx_multiply_mtx_mtx(real_t pout[], const real_t pmtxA[],
        const real_t pmtxB[],
...
extern void mpc_mtx_multiply_mtx_vec(real_t pout[], const real_t pmtx[],
        const real_t pvec[],
        const uint32_t rows,
        const uint32_t cols);
...
extern void mpc_mtx_shift_sequence(real_t outseq[],
        const real_t seq[], const uint32_t veclen, const uint32_t seqlen);

/* identity matrix for mtx with size (dimension x dimension) */
extern void mpc_mtx_to_eye(real_t mtx[], const uint32_t dimension);

/* zero matrix for mtx with nb_e entries */
extern void mpc_mtx_to_zero(real_t mtx[], const uint32_t nb_e);
